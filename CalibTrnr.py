...
def calibration(usnm,mode):
            ...
            # threshold
                ...
                if TOTAL == 0:
                    if COUNTER >= EYE_AR_CONSEC_FRAMES_MIN and COUNTER <= EYE_AR_CONSEC_FRAMES_MAX:
                        ...
                        #initialize a counter for inter blink rate after detecting first blink
                        IBT = 0
    ...
    prec = svmtrnr(usnm,mode)
    while True:
        if prec== 1:
            print("\n\n\t\t\t\t Calibration for ", mode," type blink failed,recalibrating...\n\n")
            prec = calibration(usnm,mode)
        else:
            break
    return (name, usnm, mode)


Encode ={"1111":"A","1112":"B","1121":"C","1122":"D","1211":"E","1212":"F","1221":"G","1222":"H","2111":"I","2112":"J","2121":"K","2122":"L","2211":"M","2212":"N","2221":"O","2222":"P","111":"Q","112":"R","121":"S","122":"T","211":"U","212":"V","221":"W","222":"X","1":"Y","2":"Z"}

def encrypted(op):
    lent = len(op)
    ep = ""
    if lent % 4 == 0:
        for i in range(0,lent-1,4):
            fir = i
            temp = op[fir] + op[fir+1] + op[fir+2] + op[fir+3]
            etemp = Encode[temp]
            ep = ep + etemp

        return ep
    elif lent % 4 == 1 or lent % 4 == 2:
        endl = lent % 4
        nlen = lent - endl

        for i in range(0,nlen-1,4):
            fir = i
            temp = op[fir] + op[fir+1] + op[fir+2] + op[fir+3]
            etemp = Encode[temp]
            ep = ep + etemp
        # for j in range(i+1,i+1+endl,1):
        #     ep = ep + Encode[op[j]]
        rem = op[:-endl]
        for c in rem:
            ep = ep + Encode[c]
